...
static inline int may_create(struct vfsmount *mnt, struct inode *dir, struct dentry *child)
{
	...
	if (child->d_inode && unlikely(child->d_inode->i_state & INODE_STATE_SUS_PATH) && likely(current->susfs_task_state & TASK_STRUCT_NON_ROOT_USER_APP_PROC)) {
		susfs_error = inode_permission2(mnt, dir, MAY_WRITE | MAY_EXEC);
		if (susfs_error) {
			return susfs_error;
		}
		return -ENOENT;
			...
			bool got_write, int *opened)
{
	...
	*opened &= ~FILE_CREATED;
	...
	if (IS_ERR(dentry))
		return PTR_ERR(dentry);
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!need_lookup && dentry->d_inode && unlikely(dentry->d_inode->i_state & INODE_STATE_SUS_PATH) && likely(current->susfs_task_state & TASK_STRUCT_NON_ROOT_USER_APP_PROC)) {
		dput(dentry);
		return -ENOENT;
	}
#endif
